...
def test_scenario1_basic_movement(temp_output_dir, test_dtm_dir, test_db, test_job_id):
    ...
    test_db.create_job(test_job_id)
    result = run_pipeline(test_db, test_job_id, a_path, b_path, temp_output_dir, config)
...
def test_scenario2_nodata_edges(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_scenario3_mixed_nodata_values(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_scenario4_slope_variations(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_scenario5_threshold_boundaries(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_scenario6_sparse_nodata(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_scenario7_mostly_nodata_validation_fails(test_dtm_dir, test_db, test_job_id):
    ...
    with pytest.raises(ValueError, match="insufficient valid data"):
        with tempfile.TemporaryDirectory() as tmpdir:
            run_pipeline(test_db, test_job_id, a_path, b_path, Path(tmpdir), config)


def test_scenario8_no_nodata_attribute(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_all_scenarios_generate_outputs(temp_output_dir, test_dtm_dir, test_db, test_job_id):
    ...
    for scenario_num in scenarios:
        ...
        test_db.create_job(job_id)
        result = run_pipeline(test_db, job_id, a_path, b_path, scenario_out, config)
...
def test_nodata_propagation_consistency(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_movement_rank_classification_ranges(temp_output_dir, test_dtm_dir, test_db, test_job_id):
...
def test_uncertainty_constant_outputs_exist(temp_output_dir, test_dtm_dir, test_db, test_job_id):
    a_path = test_dtm_dir / "scenario1_dem_a.tif"
    b_path = test_dtm_dir / "scenario1_dem_b.tif"
