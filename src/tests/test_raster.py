...
def test_generate_change_direction(create_test_dem):
...
def test_generate_change_magnitude(create_test_dem):
...
def test_generate_change_magnitude_with_nodata(create_test_dem):
    ...
    assert not np.ma.isMaskedArray(magnitude), (
        "Should return regular array, not masked array"
    )
    assert np.isnan(magnitude[1, 1])


def test_generate_elevation_change(create_test_dem):
    ...
    assert not np.ma.isMaskedArray(elevation_change), (
        "Should return regular array, not masked array"
    )
    assert np.allclose(elevation_change, data)


def test_generate_ranked_movement_raster(create_test_dem):
...
def test_generate_slope_degrees_raster(create_test_dem):
    ...
    # Function should return regular array (not masked)
    assert not np.ma.isMaskedArray(slope), (
        "Should return regular array, not masked array"
    )
